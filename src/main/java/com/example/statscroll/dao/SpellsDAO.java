...
public class SpellsDAO {
    ...
    public void save(Spells spell) {
        String sql = "INSERT INTO spells (id, name, level, school, cast_time, range, duration, verbal, somatic, material, material_cost, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public void update(Spells spell) {
        String sql = "UPDATE spells SET name = ?, level = ?, school = ?, cast_time = ?, range = ?, duration = ?, verbal = ?, somatic = ?, material = ?, material_cost = ?, description = ? WHERE id = ?";
        ...
        try (Connection conn = DriverManager.getConnection(url, "sa", "");
            ...
            ps.setString(11, spell.getDescription());
    ...
    private void setPreparedStatementFromSpell(PreparedStatement ps, Spells spell) throws SQLException {
        ...
        ps.setString(12, spell.getDescription());
    }
}
